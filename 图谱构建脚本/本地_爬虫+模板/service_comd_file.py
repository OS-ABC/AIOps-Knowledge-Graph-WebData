...
def get_article_url2(url,serv,tech_comd_dic,tech_server_dic,tech_file_dic):
	req = requests.get(url,headers=headers)
	content = json.loads(req.text)
	for ele in content['result_vos']:
		order_detail(ele['url'],serv,tech_comd_dic,tech_server_dic,tech_file_dic)
	req.close()

logging.basicConfig(filename = "./file/new_server_log.log",level = logging.CRITICAL)
logger = logging.getLogger()
#字典：用来存储服务与配置文件的次数关系：{"cron":{"crond":10,"vi":1}}
tech_comd_dic={}
tech_server_dic={}
tech_file_dic={}
tech_f = open("./file/my_service_test.txt",encoding='utf8')
tech_f = open("./file/serve_3.txt",encoding='utf8')
...
try:
	...
	for ele in tech_content:
		...
		url = 'https://so.csdn.net/api/v2/search?q='+tech+'&t=blog&p=1&s=0&tm=0&lv=-1&ft=0&l=&u=&platform=pc'
		print(url)
		get_article_url2(url,ele,tech_comd_dic,tech_server_dic,tech_file_dic)
