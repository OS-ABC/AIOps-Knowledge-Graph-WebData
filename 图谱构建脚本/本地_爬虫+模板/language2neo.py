...
root_node = graph.nodes.match("语言和框架",name="语言和框架").first()
...
def tech_detail(url,tech_name):
	...
	#创建图谱节点
	node=Node("语言和框架", name=tech_name)
	if not index:
		...
		while i < len(name):
			...
			i = i + 1
	# print(node)
	ab = Relationship(root_node, "包含语言或框架", node)
	graph.create(ab)
...
f=open("./file/Languages.txt","r",encoding='utf8')
try:
	content = f.read()
	...
	for ele in tech_list:
		...
		node = graph.nodes.match("语言和框架",name=ele).first()
		if not node:
